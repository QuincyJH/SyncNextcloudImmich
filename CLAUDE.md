...
`.github/workflows/publish_image.yml` builds and pushes a Docker image to ghcr.io. A "Compute image tags" step sets `image_tag` explicitly: a published **release** produces `<version>,latest` (`v` prefix stripped), a push to **main** produces `edge`. This override matters — left to its defaults `macbre/push-to-ghcr` tags releases with the version *only* and applies `latest` to non-release builds, which made `:latest` mean "tip of main". There is no test/lint CI gate.

Versioned images are only built by a published GitHub **release**, never by a bare tag push. `scripts/release.ps1` (also `make release` / `make release-dry`) resolves the next `YYYY.MM.DD.N` version, tags, pushes, and creates the release via the GitHub API; it authenticates via `gh` when installed and logged in, else falls back to `GITHUB_TOKEN`/`GH_TOKEN`, and refuses to run on a dirty tree or a `HEAD` that isn't on `origin/main`.
