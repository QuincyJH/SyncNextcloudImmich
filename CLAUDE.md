...
Versioned images are only built by a published GitHub **release**, never by a bare tag push. `scripts/release.ps1` (also `make release` / `make release-dry`) resolves the next `YYYY.MM.DD.N` version, tags, pushes, and creates the release via the GitHub API; it needs `GITHUB_TOKEN`/`GH_TOKEN` and refuses to run on a dirty tree or a `HEAD` that isn't on `origin/main`.
